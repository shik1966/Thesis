...
\begin{itemize}
    ...
    The \textbf{Full Policy} (counting empty/background slices as perfect) inflates metric values due to the large number of easy, empty-background slices, making the task appear easier than it is. In contrast, the \textbf{Simple Policy} (skipping empty slices) exposes the true difficulty of segmenting positive tumor slices, especially for diffuse and subtle tumor regions.
...
\end{center}
\noindent
Specificity for each class: Tumor Core ($0.9998$), Edema ($0.9980$), Enhancing Tumor ($0.9994$). \\
...
\begin{itemize}
    ...
    \item \textbf{Alignment with Mask:} In cases with tumor tissue, the highlighted regions in the Gabor composites visually align well with the segmented tumor areas in the mask, supporting the value of Gabor preprocessing for tumor detection.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Raw Input, Ground Truth, and Predicted Mask – Example 5.}
    \newline
    \textbf{Left:} Three‐channel MRI slice (T1ce red, T2 green, FLAIR blue) showing a clearly visible tumor region. 
    \textbf{Middle:} Ground truth mask delineating tumor core (blue), enhancing tumor (red), and edema (green). 
    \textbf{Right:} Predicted mask almost perfectly overlaps the ground truth, accurately capturing all three subregions with only minimal boundary deviation.}
    \label{fig:combined_pred_5}
\end{figure}

% Example 7: Missed Small Lesion
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{result6 3.png}
    \caption{\textbf{Raw Input, Ground Truth, and Predicted Mask – Example 6.}
    \newline
    \textbf{Left:} Three‐channel MRI slice (T1ce red, T2 green, FLAIR blue) showing a small tumor lesion.  
    \textbf{Middle:} Ground truth mask highlights a tiny enhancing tumor region (red) surrounded by edema (green).  
    \textbf{Right:} Predicted mask is completely empty, indicating the model failed to detect this subtle lesion and underscoring a limitation in sensitivity for small tumor foci.}
...
This section chronicles the systematic progression of experiments conducted in this thesis, detailing how each set of findings informed the subsequent research direction and ultimately led to the final conclusions about feature detection techniques in brain tumor segmentation. Each experiment's complete evaluation metrics are presented to provide a comprehensive understanding of the performance evolution across different preprocessing approaches.
...
\paragraph{Comprehensive Performance Metrics}

\textbf{Full Policy Results (including empty slices):}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.8353 ± 0.2871, IoU = 0.7953 ± 0.3223, HD95 = 2.62 ± 6.32, ASSD = 0.75 ± 1.65
    \item \textbf{Edema}: Dice = 0.6336 ± 0.3932, IoU = 0.5770 ± 0.3997, HD95 = 7.64 ± 12.72, ASSD = 2.46 ± 4.73
    \item \textbf{Enhancing Tumor}: Dice = 0.8730 ± 0.2458, IoU = 0.8322 ± 0.2705, HD95 = 4.06 ± 13.88, ASSD = 1.41 ± 7.12
\end{itemize}

\textbf{Simple Policy Results (tumor-containing slices only):}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.5168 ± 0.2966, IoU = 0.3993 ± 0.2587, HD95 = 8.60 ± 8.93, ASSD = 2.48 ± 2.17
    \item \textbf{Edema}: Dice = 0.4181 ± 0.3465, IoU = 0.3281 ± 0.2942, HD95 = 14.12 ± 14.40, ASSD = 4.55 ± 5.64
    \item \textbf{Enhancing Tumor}: Dice = 0.6572 ± 0.2985, IoU = 0.5469 ± 0.2614, HD95 = 11.89 ± 21.72, ASSD = 4.14 ± 11.71
\end{itemize}

\textbf{Pixel-Level Metrics:}
\begin{itemize}
    \item \textbf{Tumor Core}: Precision = 0.805, Recall = 0.555, Specificity = 0.9998, Avg FP/slice = 14.2, Avg FN/slice = 47.0
    \item \textbf{Edema}: Precision = 0.700, Recall = 0.744, Specificity = 0.9976, Avg FP/slice = 137.7, Avg FN/slice = 110.9
    \item \textbf{Enhancing Tumor}: Precision = 0.733, Recall = 0.837, Specificity = 0.9993, Avg FP/slice = 38.8, Avg FN/slice = 20.8
\end{itemize}

\paragraph{Critical Insight} The baseline results revealed substantial performance gaps between Full and Simple policies, with Dice scores dropping by 38-34\% when empty slices were excluded. This finding, combined with high false positive rates for edema (137.7/slice) and low recall for tumor core (0.555), motivated the exploration of classical feature detection techniques to enhance boundary and texture information.
...
\textbf{Full Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.8027 ± 0.3360, IoU = 0.7709 ± 0.3625, HD95 = 2.03 ± 4.10, ASSD = 0.63 ± 1.31
    \item \textbf{Edema}: Dice = 0.5979 ± 0.4103, IoU = 0.5448 ± 0.4108, HD95 = 6.40 ± 10.29, ASSD = 2.43 ± 7.09
    \item \textbf{Enhancing Tumor}: Dice = 0.8530 ± 0.2625, IoU = 0.8093 ± 0.2936, HD95 = 1.38 ± 2.74, ASSD = 0.41 ± 0.72
\end{itemize}

\textbf{Simple Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.4080 ± 0.3242, IoU = 0.3126 ± 0.2813, HD95 = 7.75 ± 4.46, ASSD = 2.42 ± 1.50
    \item \textbf{Edema}: Dice = 0.3688 ± 0.3459, IoU = 0.2854 ± 0.2820, HD95 = 12.54 ± 11.43, ASSD = 4.76 ± 9.35
    \item \textbf{Enhancing Tumor}: Dice = 0.5950 ± 0.2921, IoU = 0.4745 ± 0.2481, HD95 = 4.29 ± 3.29, ASSD = 1.26 ± 0.72
\end{itemize}

\textbf{Pixel-Level Metrics:}
\begin{itemize}
    \item \textbf{Tumor Core}: Precision = 0.8057, Recall = 0.4325, Specificity = 0.9998, Avg FP/slice = 11.01, Avg FN/slice = 59.94
    \item \textbf{Edema}: Precision = 0.7146, Recall = 0.6754, Specificity = 0.9980, Avg FP/slice = 116.69, Avg FN/slice = 140.46
    \item \textbf{Enhancing Tumor}: Precision = 0.7127, Recall = 0.7074, Specificity = 0.9994, Avg FP/slice = 36.41, Avg FN/slice = 37.36
\end{itemize}

\paragraph{Performance Degradation} Contrary to expectations, Sobel filtering led to decreased performance across all tumor classes under the Simple Policy compared to raw intensities. Most notably, tumor core recall dropped from 0.555 to 0.4325, indicating that edge enhancement made the model more conservative in detection.

\paragraph{Critical Realization} The Sobel results indicated that pure edge detection was removing important intensity and texture information that the network needed for effective segmentation. This finding suggested that tumor segmentation required more than just boundary information.

\subsection{Stage 3: Gabor Texture and Orientation Analysis}

\subsubsection{Rationale for Gabor Filtering}

The transition to Gabor filtering was motivated by the need to capture texture and orientation-dependent features that Sobel filtering had overlooked. The hypothesis was that brain tumors exhibit distinctive texture patterns that would be enhanced by multi-scale, multi-orientation Gabor filtering.

\subsubsection{Gabor Results: A Concerning Decline}

\textbf{Full Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.7189 ± 0.4081, IoU = 0.6956 ± 0.4252, HD95 = 6.24 ± 15.68, ASSD = 2.91 ± 8.88
    \item \textbf{Edema}: Dice = 0.5239 ± 0.4303, IoU = 0.4793 ± 0.4322, HD95 = 17.26 ± 24.71, ASSD = 8.54 ± 15.86
    \item \textbf{Enhancing Tumor}: Dice = 0.7273 ± 0.3748, IoU = 0.6868 ± 0.3974, HD95 = 8.97 ± 20.82, ASSD = 3.94 ± 11.45
\end{itemize}

\textbf{Simple Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.2093 ± 0.2559, IoU = 0.1439 ± 0.1901, HD95 = 23.76 ± 22.81, ASSD = 11.08 ± 14.49
    \item \textbf{Edema}: Dice = 0.2612 ± 0.3053, IoU = 0.1920 ± 0.2402, HD95 = 31.79 ± 25.74, ASSD = 15.73 ± 18.71
    \item \textbf{Enhancing Tumor}: Dice = 0.3427 ± 0.2928, IoU = 0.2450 ± 0.2170, HD95 = 25.86 ± 28.49, ASSD = 11.35 ± 17.14
\end{itemize}

\textbf{Pixel-Level Metrics:}
\begin{itemize}
    \item \textbf{Tumor Core}: Precision = 0.3980, Recall = 0.3371, Specificity = 0.9991, Avg FP/slice = 53.85, Avg FN/slice = 70.01
    \item \textbf{Edema}: Precision = 0.6778, Recall = 0.4493, Specificity = 0.9984, Avg FP/slice = 92.40, Avg FN/slice = 238.27
    \item \textbf{Enhancing Tumor}: Precision = 0.3981, Recall = 0.5821, Specificity = 0.9980, Avg FP/slice = 112.36, Avg FN/slice = 53.35
\end{itemize}

\paragraph{Severe Performance Degradation} The Gabor experiments produced the worst results of the entire study. Under Simple Policy, tumor core Dice plummeted to 0.2093 (59\% decrease from baseline), with precision dropping to 0.398. The high false positive rate for enhancing tumor (112.36/slice) suggested severe over-segmentation issues.

\paragraph{Over-Specialization Problem} Gabor filters appeared to create feature representations that were too abstract for the SegNet architecture to effectively utilize, leading to both poor precision and recall across all classes.

\subsection{Stage 4: Laplacian Multi-Scale Edge and Blob Detection}

\subsubsection{Strategic Pivot to Laplacian Filtering}

The disappointing Gabor results necessitated a strategic pivot. Laplacian-of-Gaussian (LoG) filtering was selected as it could capture both edge and blob-like structures simultaneously while maintaining better relationships to intensity patterns.

\subsubsection{Laplacian Results: Recovery and Optimization}

\textbf{Full Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.7938 ± 0.3364, IoU = 0.7593 ± 0.3671, HD95 = 2.05 ± 3.77, ASSD = 0.75 ± 1.38
    \item \textbf{Edema}: Dice = 0.5827 ± 0.4050, IoU = 0.5264 ± 0.4102, HD95 = 12.21 ± 18.76, ASSD = 5.38 ± 11.59
    \item \textbf{Enhancing Tumor}: Dice = 0.8410 ± 0.2643, IoU = 0.7931 ± 0.3031, HD95 = 1.48 ± 2.71, ASSD = 0.48 ± 0.76
\end{itemize}

\textbf{Simple Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.3815 ± 0.2908, IoU = 0.2780 ± 0.2381, HD95 = 7.32 ± 3.47, ASSD = 2.67 ± 1.30
    \item \textbf{Edema}: Dice = 0.3372 ± 0.3129, IoU = 0.2478 ± 0.2402, HD95 = 22.56 ± 20.41, ASSD = 9.93 ± 14.24
    \item \textbf{Enhancing Tumor}: Dice = 0.5620 ± 0.2650, IoU = 0.4301 ± 0.2150, HD95 = 4.60 ± 2.90, ASSD = 1.48 ± 0.54
\end{itemize}

\textbf{Pixel-Level Metrics:}
\begin{itemize}
    \item \textbf{Tumor Core}: Precision = 0.7773, Recall = 0.3975, Specificity = 0.9998, Avg FP/slice = 12.03, Avg FN/slice = 63.64
    \item \textbf{Edema}: Precision = 0.5840, Recall = 0.6608, Specificity = 0.9964, Avg FP/slice = 203.66, Avg FN/slice = 146.76
    \item \textbf{Enhancing Tumor}: Precision = 0.6061, Recall = 0.7415, Specificity = 0.9989, Avg FP/slice = 61.53, Avg FN/slice = 33.00
\end{itemize}

\paragraph{Performance Recovery} Laplacian filtering demonstrated significant recovery from Gabor's poor performance. Tumor core achieved the highest precision (0.7773) among all feature detection methods, while enhancing tumor showed excellent recall (0.7415). However, edema false positives increased dramatically (203.66/slice).

\subsection{Stage 5: Combined Feature Detection Approach}

\subsubsection{Motivation for Integration}

The individual experiments revealed that each feature detection method had unique strengths: Sobel for edges, Gabor for textures, and Laplacian for multi-scale structures. Additionally, the dataset was expanded from 900 to approximately 11,000 slices to provide more robust training.

\subsubsection{Combined Results: Synergistic Performance}

\textbf{Full Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.8170 ± 0.3145, IoU = 0.7794 ± 0.3390, HD95 = 3.66 ± 7.69, ASSD = 1.20 ± 2.81
    \item \textbf{Edema}: Dice = 0.5895 ± 0.4116, IoU = 0.5361 ± 0.4109, HD95 = 6.97 ± 11.39, ASSD = 2.23 ± 4.70
    \item \textbf{Enhancing Tumor}: Dice = 0.8425 ± 0.2914, IoU = 0.8050 ± 0.3117, HD95 = 2.77 ± 7.23, ASSD = 0.93 ± 3.00
\end{itemize}

\textbf{Simple Policy Results:}
\begin{itemize}
    \item \textbf{Tumor Core}: Dice = 0.5258 ± 0.3440, IoU = 0.4285 ± 0.3118, HD95 = 10.72 ± 9.88, ASSD = 3.51 ± 3.88
    \item \textbf{Edema}: Dice = 0.3807 ± 0.3554, IoU = 0.3002 ± 0.2993, HD95 = 12.82 ± 12.78, ASSD = 4.11 ± 5.73
    \item \textbf{Enhancing Tumor}: Dice = 0.5899 ± 0.3428, IoU = 0.4923 ± 0.3069, HD95 = 8.25 ± 10.50, ASSD = 2.78 ± 4.66
\end{itemize}

\textbf{Pixel-Level Metrics:}
\begin{itemize}
    \item \textbf{Tumor Core}: Precision = 0.661, Recall = 0.679, Specificity = 0.9988, Avg FP/slice = 80.49, Avg FN/slice = 74.08
    \item \textbf{Edema}: Precision = 0.712, Recall = 0.676, Specificity = 0.9977, Avg FP/slice = 149.28, Avg FN/slice = 176.86
    \item \textbf{Enhancing Tumor}: Precision = 0.757, Recall = 0.716, Specificity = 0.9992, Avg FP/slice = 55.14, Avg FN/slice = 67.89
\end{itemize}

\paragraph{Optimal Balance Achievement} The combined approach achieved the best overall performance, with tumor core Simple Policy Dice improving to 0.5258 (slightly better than baseline). The precision-recall balance was notably improved, with enhancing tumor achieving both high precision (0.757) and recall (0.716).

\subsection{Synthesis: Evolution of Understanding}

\subsubsection{Comparative Performance Analysis}

Examining the complete metric evolution across all experiments reveals several key patterns:

\textbf{Simple Policy Dice Coefficient Evolution:}
\begin{itemize}
    \item \textbf{Tumor Core}: Raw (0.517) → Sobel (0.408) → Gabor (0.209) → Laplacian (0.382) → Combined (0.526)
    \item \textbf{Edema}: Raw (0.418) → Sobel (0.369) → Gabor (0.261) → Laplacian (0.337) → Combined (0.381)
    \item \textbf{Enhancing Tumor}: Raw (0.657) → Sobel (0.595) → Gabor (0.343) → Laplacian (0.562) → Combined (0.590)
\end{itemize}

\textbf{Precision-Recall Trade-offs:}
\begin{itemize}
    \item Raw intensities provided high precision but variable recall
    \item Sobel maintained precision but reduced recall
    \item Gabor severely compromised both metrics
    \item Laplacian recovered precision at the cost of increased false positives
    \item Combined approach achieved the best balance across all metrics
\end{itemize}

\subsubsection{Key Insights and Implications}

The complete experimental timeline revealed that:

\begin{enumerate}
    \item \textbf{Baseline Robustness}: Raw intensities provided surprisingly strong performance, particularly for enhancing tumor detection
    \item \textbf{Feature Complexity Paradox}: More sophisticated features (Gabor) did not guarantee better performance
    \item \textbf{Scale Importance}: Both dataset scale (900 vs 11,000 slices) and feature scale (multi-scale LoG) were crucial
    \item \textbf{Complementary Integration}: Combining multiple feature types leveraged their individual strengths while mitigating weaknesses
    \item \textbf{Persistent Challenges}: Edema segmentation remained difficult across all approaches, with consistently high false positive rates
\end{enumerate}

This comprehensive analysis demonstrates that while classical feature detection can enhance segmentation, the benefits are modest and highly dependent on careful implementation, with the combined multi-feature approach on expanded data providing the most robust solution.
