...
\addbibresource{bachelor.bib} % Adjust to your actual filename
...
\subsubsection*{Cell 1: Mount Google Drive in Colab}
...
\subsubsection*{Cell 2: Force-remount Google Drive in Colab}
...
\subsubsection*{Cell 3: Imports and Data Paths Setup}
...
\subsubsection*{Cell 4: Build Raw $256\times256$ Dataset from NIfTI Volumes}

\begin{lstlisting}[language=Python]
# Cell 4: Build Raw 256×256 Dataset from NIfTI Volumes (no additional preprocessing)
# Purpose: Load the three MRI modalities and segmentation masks slice-by-slice,
#          resize to 256×256, stack the modalities into a 3-channel array,
...
if os.path.exists(X_PATH_RAW) and os.path.exists(Y_PATH_RAW):
    ...
    print("⚙️  Building raw dataset from patient folders (no Sobel)…")
    Xs, Ys = [], []
...
\subsubsection*{Cell 5: Subsample 30 Patients}
...
#          Stack the three raw‐intensity modalities (no Sobel) into a (H×W×3) array and save.
...
if (not FORCE_REBUILD) and os.path.exists(X_PATH_SUB) and os.path.exists(Y_PATH_SUB):
    ...
    print("⚙️  Building custom subsampled raw dataset …")
...
\subsubsection*{Cell 6: Visualizing Raw Subsampled Slices}

\begin{lstlisting}[language=Python]
# Cell 6 (v_raw): Memory-map Raw Subsampled Dataset and Show Random Samples
# Purpose: Load the “no-resize” subsampled raw arrays (H×W×3), normalize per-channel,
...
\subsubsection*{Cell 7: Train/Validation/Test Split of the Subsampled Dataset}
...
# 4) Of the remaining 85%, take ~17.65% (≈15% of original 900) for validation
#    17.65% of 85% ≈ 15% overall. So test_size = 0.1765 here.
...
\subsubsection*{Cell 8: Verifying Multi-Class Mask Values and Shapes}
...
\subsubsection*{Cell 9: Simple Data Generator}
...
\subsubsection*{Cell 10: Instantiate Generators for Train, Validation, and Test Splits}
...
\subsubsection*{Cell 11: Loss Functions for Binary and Multi-Class Segmentation}
...
def multiclass_dice_loss(y_true, y_pred, smooth=1e-6):
    ...
    We flatten both to shape (total_pixels, 4), cast y_true→float32, then compute per‐class Dice.
    """
...
\subsubsection*{Cell 12: SegNet\_Multiclass\_Enhanced (Four-Class Segmentation)}
...
def segnet_multiclass_enhanced(input_shape=(240,240,3), n_classes=4, dropout=0.2):
    ...
    d1 = conv_block(d1, 64)
    # Final 1×1 convolution → 4-channel softmax
...
\FloatBarrier
 ...
 \subsubsection*{Cell 13: Compile, Train, Save Model \& History (4-Class SegNet)}
...
100%
 20/20 [10:08<00:00, 28.83s/epoch, accuracy=0.995, loss=0.233, multiclass_dice_coef=0.792, val_accuracy=0.995, val_loss=0.222, val_multiclass_dice_coef=0.799]
...
Epoch 20/20
...
\subsubsection*{Cell 14: Load Best Model and Compute Per‐Class Dice on Test Set}
...
Evaluating Dice: 100%|██████████| 135/135 [01:00<00:00,  2.24it/s]
�� Average Dice Score per Class:
...
\subsubsection*{Cell 15: Compute Slicewise Dice, IoU, HD95 \& ASSD under Empty-Slice Policies}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{\textbf{Sample 1.} \emph{Left:} Raw-intensity RGB composite showing preserved anatomical structure. \emph{Right:} Segmentation mask indicates tumor presence and boundaries.}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{subsample2_no.png}
        \caption{\textbf{Sample 2.} \emph{Left:} Raw-intensity slice with moderate intensity variation. \emph{Right:} Mask shows corresponding tumor region.}
    \end{minipage}
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{\textbf{Sample 3.} \emph{Left:} Raw-intensity RGB image with visible normal brain anatomy. \emph{Right:} Segmentation mask for a slice with a small, peripheral lesion.}
    \end{minipage}
...
\textbf{Metric Values (Mean, Per Class):}

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Class} & \textbf{Dice (Full)} & \textbf{IoU (Full)} & \textbf{HD95 (Full)} & \textbf{ASSD (Full)} \\
\midrule
Tumor Core       & 0.8353 & 0.7953 & 2.62 & 0.75 \\
Edema            & 0.6336 & 0.5770 & 7.64 & 2.46 \\
Enhancing Tumor  & 0.8730 & 0.8322 & 4.06 & 1.41 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Class} & \textbf{Dice (Simple)} & \textbf{IoU (Simple)} & \textbf{HD95 (Simple)} & \textbf{ASSD (Simple)} \\
\midrule
Tumor Core       & 0.5168 & 0.3993 & 8.60 & 2.48 \\
Edema            & 0.4181 & 0.3281 & 14.12 & 4.55 \\
Enhancing Tumor  & 0.6572 & 0.5469 & 11.89 & 4.14 \\
\bottomrule
\end{tabular}
...
\newgeometry{left=0cm, right=0.2cm, top=0cm, bottom=0cm}
\begin{figure}[p]
    \centering
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{truth_no.png}
...
\begin{figure}[H]
    ...
    \newline
    \textbf{Left:} Raw-intensity RGB input (normalized).
    \textbf{Right:} Segmentation mask showing a large, heterogeneous tumor region—raw intensities provide a realistic view of inherent contrast and tumor complexity.}
...
\begin{figure}[H]
    ...
    \newline
    \textbf{Left:} Raw-intensity RGB with visible anatomical features.
    \textbf{Right:} Mask contains multiple subregions, demonstrating sensitivity to both major and minor tumor components in the unfiltered data.}
...
\begin{figure}[H]
    ...
    \newline
    \textbf{Left:} Raw RGB input, anatomical complexity preserved.
    \textbf{Right:} Mask shows multiple, spatially distinct tumor regions—model faces the full variability of MRI presentation without preprocessing.}
...
\begin{figure}[H]
    ...
    \newline
    \textbf{Left:} Raw RGB input, predominantly background with brain edge.
    \textbf{Right:} Mask is empty, exemplifying specificity to normal structure and absence of false positives in non-tumor slices.}
...
\begin{figure}[H]
    ...
    \newline
    \textbf{Left:} Raw RGB input (normalized), clear anatomical and pathological contrast.
    \textbf{Right:} Mask displays a single, sharply defined tumor region, highlighting model ability to localize true positives even in unfiltered inputs.}
...
\textbf{\# Cell 16: Plot Sobel Training History (Accuracy, Loss, Dice)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Dice coefficient.png}
    \caption{\textbf{Multiclass Dice Coefficient vs. Epoch.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.97\textwidth]{accuracy and loss.png}
    \caption{\textbf{Left: Multiclass Accuracy vs. Epoch. Right: Multiclass Loss vs. Epoch.}
    The left plot shows that training and validation accuracy both improve rapidly in early epochs and plateau near 99\%, with validation accuracy exhibiting early spikes due to limited data and class imbalance. The right plot presents the multiclass loss, which decreases sharply for both training and validation sets in the first few epochs, then stabilizes at low values, confirming effective learning. Validation loss is initially high and erratic but follows a downward trend similar to training loss. Together, these metrics confirm successful training and generalization of the SegNet model with Sobel‐filtered inputs.}
...
\textbf{Quantitative Summary (mean):}
...
\textbf{Summary:}  
Under the Full Policy, Sobel preprocessing achieves its best segmentation performance on the Enhancing Tumor class (Dice = 0.8530, IoU = 0.8093) and the lowest on Edema (Dice = 0.5979, IoU = 0.5448), with very tight boundary adherence reflected in low HD95 and ASSD values. When evaluated only on tumor‐containing slices (Simple Policy), performance declines notably across all classes—most dramatically for Tumor Core (Dice drops to 0.4080) and Edema (Dice drops to 0.3688)—highlighting the increased challenge of precise tumor delineation when empty slices are excluded. This contrast underscores the need for robust preprocessing to handle the variability of slice content in clinical applications.  
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{Metrics.png}
...
\textbf{\# Cell 16: Plot Gabor Training History (Accuracy, Loss, Dice)}

This cell loads the training history from disk and plots the evolution of accuracy, loss, and Dice coefficient for both training and validation sets over 20 epochs of training the SegNet model on the Gabor-filtered dataset.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{dice_gabor.png}
    \caption{\textbf{Multiclass Dice Coefficient vs. Epoch (Gabor Filtered).}
    This plot displays the multiclass Dice coefficient on both training and validation sets for each epoch. Both curves show steady improvement, with the validation Dice closely tracking the training trend but exhibiting higher fluctuations. This pattern indicates that the network learns to segment tumor regions more accurately over time, though validation Dice may fluctuate due to dataset size and the inherent complexity of brain tumor segmentation.}
    \label{fig:gabor_dice}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{accuracy and loss_gabor.png}
    \caption{\textbf{Left: Multiclass Accuracy vs. Epoch. Right: Multiclass Loss vs. Epoch (Gabor Filtered).}
    The left panel shows that both training and validation accuracy improve rapidly in the initial epochs and approach near-perfect values, with occasional fluctuations due to data variability. The right panel shows the multiclass loss, which decreases sharply for both training and validation sets, confirming that the model is learning effectively on the Gabor-filtered data. The trends are consistent with effective training and reasonable generalization on the validation set.}
...
\textbf{Metric Values (Mean ± Std, Per Class):}
\vspace{1em}
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{metrics_gabor.png}
...
def apply_laplacian(slice_2d: np.ndarray,
    ...
    Parameters:
      ...
      - sigmas:   sequence of Gaussian σ values for pre‐blurring
      - ksize:    kernel size for the Laplacian (odd integer ≥ 1)

    Returns:
      A float32 array of combined Laplacian responses (H×W).
    """
...
\textbf{\# Cell 16: Plot Laplacian Training History (Accuracy, Loss, Dice)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width= 1\textwidth]{dice_laplacian.png}
    \caption{\textbf{Multiclass Dice Coefficient vs. Epoch (Laplacian Filtered).}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{accuracy and loss_laplacian.png}
    \caption{\textbf{Left: Multiclass Accuracy vs. Epoch. Right: Multiclass Loss vs. Epoch (Laplacian Filtered).}
...
\noindent\textbf{Simple Policy:}

\begin{table}[H]
  \centering
  \label{tab:sobel_simple_policy}
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Class} & \textbf{Dice (Simple)} & \textbf{IoU (Simple)} & \textbf{HD95 (Simple)} & \textbf{ASSD (Simple)} \\
    \midrule
...
\vspace{1em}
\textbf{Summary:}  
Under the Full Policy, segmentation performance is strongest for the Enhancing Tumor class (Dice = 0.8410, IoU = 0.7931) and weakest for Edema (Dice = 0.5827, IoU = 0.5264), with relatively low boundary errors. When evaluated only on tumor‐containing slices (Simple Policy), all classes exhibit marked performance drops—most dramatically for Edema (Dice falls to 0.3372 and HD95 rises to 22.56, ASSD to 9.93)—highlighting the increased difficulty of delineating edema in a clinically realistic setting. Tumor Core also declines substantially (Dice = 0.3815), underscoring the need for further model and preprocessing enhancements to handle challenging tumor subtypes.  
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{results_laplacian.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{sub8.png}
    \caption{\textbf{Combined Features Sample 8.} Effective delineation of tumor boundary enhanced by texture and edge information.}
    \label{fig:combined_sub8}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{sub10.png}
    \caption{\textbf{Combined Features Sample 10.} Strong visual correlation between preprocessed features and segmentation mask, even for challenging scenarios.}
    \label{fig:combined_sub10}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{truths 3.png}
